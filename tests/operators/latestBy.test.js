...
describe('latestBy operators ', () => {
    ...
    test('latestBy should cancel execution of previous matching passed values', async() => {
        const results = await provider({ flatten: [ { name: 'Kim', age: 15, }, { name: 'John', age: 20, }, { name: 'Lisa', age: 25, }, { name: 'John', age: 25, }, ], })
            .latestBy([ 'name', ])
            .delay(10)
    ...
    test('latest with multiple keys', async() => {
        const results = await provider({ flatten: [ { name: 'John', age: 20, }, { name: 'Lisa', age: 25, }, { name: 'John', age: 25, }, { name: 'John', age: 20, }, ], })
            .latestBy([ 'name', 'age', ])
            .delay(10)
            .reduce((acc, next) => [ ...acc, next, ], [])
            .pull();
