...
describe('operator some', () => {
    ...
    test('some async should return false', async () => {
        const result = await provider.fromIterable( [ sleepAndReturn(10, 6), sleepAndReturn(15, 10), ])
            .await()
            .some(it => it<5)
            .pull();
        expect(result).toBe(false);
    ...
    test('some sync should return false', async () => {
        const result = await provider.fromIterable([ 6, 10, ])
            .some(it => it<5)
            .pull();
        expect(result).toBe(false);
    });
    ...
    test('some sync should return true', async () => {
        const result = await provider.fromIterable([ 6, 10, 4, 3, ])
            .some(it => it<5)
            .pull();
        expect(result).toBe(true);
    });
